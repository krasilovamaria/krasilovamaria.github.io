...
/* harmony import */ var _vendor_svgxuse__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./vendor/svgxuse */ "./js/vendor/svgxuse.js");
/* harmony import */ var _vendor_svgxuse__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_vendor_svgxuse__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _vendor_picturefill__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./vendor/picturefill */ "./js/vendor/picturefill.js");
/* harmony import */ var _vendor_picturefill__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_vendor_picturefill__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _vendor_shapes_polyfill__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./vendor/shapes-polyfill */ "./js/vendor/shapes-polyfill.js");
/* harmony import */ var _vendor_shapes_polyfill__WEBPACK_IMPORTED_MODULE_2___default = /*#__PURE__*/__webpack_require__.n(_vendor_shapes_polyfill__WEBPACK_IMPORTED_MODULE_2__);
...
(function (scope) {
  ...
  function Metrics(element) {
    var computedStyle = getComputedStyle(element); // a full units map would call getUnitsMap, but that can significantly slow things down

    this.units = {
      px: 1
    };
    this.element = element; // Used values already in px, but may be "" in some browsers, eg FF
    // These values are stored in their CSS order top, right, bottom, left

    var parseLength = function parseLength(length) {
      return length && length.length ? parseInt(length) : 0;
    };
    ...
    this.borderBox.radii = radii.map(function (radius, index) {
      ...
      return [self.toPixels(radius[0], self.borderBox.width), self.toPixels(radius.length > 1 ? radius[1] : radius[0], self.borderBox.height)];
    });
  ...
  Metrics.prototype.getUnitsMap = function (element) {
    ...
    units.forEach(function (unit) {
      child.style.lineHeight = '1' + unit; // computed height is in px
  ...
  Metrics.prototype.toPixels = function (length, percentageBase) {
    ...
    if (!split[2]) return split[1];
    if (split[2] === '%') return split[1] * percentageBase / 100;
  ...
  function isPointOnLineSegment(lineStartPoint, lineEndPoint, point) {
    return point.x >= Math.min(lineStartPoint.x, lineEndPoint.x) && point.x <= Math.max(lineStartPoint.x, lineEndPoint.x) && areCollinearPoints(lineStartPoint, lineEndPoint, point);
  }
  ...
  PolygonEdge.prototype.overlapsYRange = function (y1, y2) {
    // y range: y1 <= y <= y2
  ...
  PolygonEdge.prototype.isWithinYRange = function (y1, y2) {
    // y range: y1 <= y <= y2
    var edgeY1 = this.vertex1.y;
    var edgeY2 = this.vertex2.y;
  ...
  PolygonEdge.prototype.xIntercept = function (y) {
    ...
    if (vertex1Y == vertex2Y) return Math.min(this.vertex1.x, this.vertex2.x);
    if (y == Math.min(vertex1Y, vertex2Y)) return vertex1Y < vertex2Y ? this.vertex1.x : this.vertex2.x;
    if (y == Math.max(vertex1Y, vertex2Y)) return vertex1Y > vertex2Y ? this.vertex1.x : this.vertex2.x;
  ...
  */
  ...
  PolygonEdge.prototype.clippedEdgeXRange = function (y1, y2) {
    if (this.isWithinYRange(y1, y2)) {
      ...
      return {
        x1: Math.min(vertex1X, vertex2X),
        x2: Math.max(vertex1X, vertex2X)
      };
    }
    ...
    return {
      x1: Math.min(xForY1, xForY2),
      x2: Math.max(xForY1, xForY2)
    };
  };
  ...
  */
  ...
  function clippedCircleXRange(center, radius, y1, y2) {
    if (center.y >= y1 && center.y <= y2) return {
      x1: center.x - radius,
      x2: center.x + radius
    };
    var yi, xi;
  ...
  function isVertexOrderClockwise(vertices) {
    ...
    for (var i = 1; i < vertices.length; ++i) {
      var p = vertices[i];
      if (p.y < vertices[minVertexIndex].y || p.y == vertices[minVertexIndex].y && p.x < vertices[minVertexIndex].x) minVertexIndex = i;
    }
  ...
  function Polygon(vertices, fillRule, shapeMargin) {
    // vertices:  [{x, y}]
    ...
    } while (vertex1Index !== 0); // Where possible, combine 2 edges into 1
  ...
  Polygon.prototype.edgeAt = function (index) {
  ...
  Polygon.prototype.overlapsYRange = function (y1, y2) {
    // y range: y1 <= y < y2
  ...
  Polygon.prototype.nextEdgeVertexIndex = function (vertex1Index, clockwise) {
    ...
    while (vertex2Index) {
      ...
      if (!areCollinearPoints(this.vertexAt(vertex1Index), this.vertexAt(vertex2Index), this.vertexAt(vertexIndex3))) break;
  ...
  Polygon.prototype.containsPointEvenOdd = function (point) {
    ...
    for (var i = 0; i < this.m_edges.length; ++i) {
      ...
      if (edge.containsPoint(point)) return true;
      ...
      if (vertex1.y <= point.y && vertex2.y > point.y || vertex1.y > point.y && vertex2.y <= point.y) {
        ...
        if (point.x < vertex1.x + vt * (vertex2.x - vertex1.x)) ++crossingCount;
      }
    }
  ...
  Polygon.prototype.containsPointNonZero = function (point) {
    ...
    for (var i = 0; i < this.m_edges.length; ++i) {
      ...
      if (vertex2.y < point.y) {
        if (vertex1.y > point.y && compareLineSlopes(vertex1, vertex2, point) > 0) ++windingNumber;
  ...
  Polygon.prototype.edgeVerticesThatOverlapYRange = function (y1, y2) {
    ...
    for (var i = 0; i < this.m_edges.length; i++) {
      ...
      if (vertex.y >= y1 && vertex.y < y2) result.push(vertex);
    }
  ...
  Polygon.prototype.edgesThatOverlapYRange = function (y1, y2) {
    ...
    for (var i = 0; i < this.m_edges.length; i++) {
      var edge = this.edgeAt(i);
      if (edge.overlapsYRange(y1, y2)) result.push(edge);
    }
  ...
  function compareEdgeMinX(edge1, edge2) {
    return edge1.minX - edge2.minX;
  }

  function compareVertexXIncreasing(vertex1, vertex2) {
    return vertex2.x - vertex1.x;
  }

  Polygon.prototype.leftExclusionEdge = function (y1, y2) {
    // y2 >= y1
    if (this.isEmpty() || !this.bounds.overlapsYRange(y1, y2)) return undefined;
    var result, i, xRange;
    ...
    if (overlappingEdges.length !== 0) {
      ...
      for (i = 1; i < overlappingEdges.length; i++) {
        if (overlappingEdges[i].minX > result) break;
    ...
    if (result === undefined) console.error("Polygon leftExclusionEdge() failed");
    return result;
  };

  function compareEdgeMaxX(edge1, edge2) {
    return edge2.maxX - edge1.maxX;
  }

  function compareVertexXDecreasing(vertex1, vertex2) {
    return vertex1.x - vertex2.x;
  }

  Polygon.prototype.rightExclusionEdge = function (y1, y2) {
    // y2 >= y1
    if (this.isEmpty() || !this.bounds.overlapsYRange(y1, y2)) return undefined;
    var result, i, xRange;
    var overlappingEdges = this.shapeMarginEdgesThatOverlapYRange(y1, y2);
    ...
    if (overlappingEdges.length !== 0) {
      ...
      for (i = 1; i < overlappingEdges.length; i++) {
        if (overlappingEdges[i].maxX < result) break;
        xRange = overlappingEdges[i].clippedEdgeXRange(y1, y2);
    ...
    if (result === undefined) console.error("Polygon rightExclusionEdge() failed");
    return result;
  };
  ...
  RasterIntervals.prototype.intervalAt = function (y) {
    return this.intervals[y + this.yOffset];
  };

  RasterIntervals.prototype.setIntervalAt = function (y, value) {
    this.intervals[y + this.yOffset] = value;
  };

  RasterIntervals.prototype.uniteIntervalAt = function (y, interval) {
    var intervalAtY = this.intervalAt(y);
    if (intervalAtY === RasterIntervals.none) this.setIntervalAt(y, interval);else {
  ...
  RasterIntervals.prototype.intervalAtContains = function (y, interval) {
    var intervalAtY = this.intervalAt(y);
    if (intervalAtY == RasterIntervals.none) return false;
  ...
  RasterIntervals.prototype.computeMarginIntervals = function (shapeMargin, clip) {
    ...
    for (var y = this.minY; y < this.maxY; ++y) {
      var intervalAtY = this.intervalAt(y);
      if (intervalAtY == RasterIntervals.none) continue;
      ...
      for (marginY = y - 1; marginY >= marginY0; --marginY) {
        if (marginY > 0 && this.intervalAtContains(marginY, intervalAtY)) break;
      ...
      for (marginY = y + 1; marginY <= marginY1; ++marginY) {
        if (marginY < this.maxY && this.intervalAtContains(marginY, intervalAtY)) break;
        result.uniteIntervalAt(marginY, mig.generateIntervalAt(marginY, intervalAtY));
      }
    }
  ...
  function error(url, exception) {
    ...
    if (exception) console.log("The exact problem was ", exception);
  }
  ...
  RasterImage.prototype.hasData = function () {
  ...
  Raster.prototype.init = function (callback) {
    ...
    image.onload = function () {
      ...
      if (raster.intervals) {
        if (raster.shapeMargin > 0) raster.intervals = raster.intervals.computeMarginIntervals(raster.shapeMargin, raster.clip);
      }
    ...
    if (!image.hasOwnProperty('crossOrigin') && window.URL && window.URL.createObjectURL) {
  ...
  Raster.prototype.computeIntervals = function (image) {
    ...
    if (!rasterImage.hasData()) return undefined;
    ...
    for (var y = 0; y < maxY; y++) {
      ...
      for (var x = 0; x < this.box.width; x++) {
        ...
        if (startX == -1 && alpha > threshold) {
          startX = x;
          if (intervals.intervalAt(y) === RasterIntervals.none) intervals.setIntervalAt(y, new RasterInterval(y, startX, width));
  ...
  Raster.prototype.rightExclusionEdge = function (y1, y2) {
    // y2 >= y1
    var intervals = this.intervals;
    if (!intervals) return this.clip.width;
    var x; // = undefined;

    for (var y = Math.max(y1, this.clip.y); y <= y2 && y < this.clip.maxY; y++) {
      var endX = intervals.intervalAt(y).endX;
      if (x === undefined || endX !== undefined && endX > x) x = endX;
    }

    return x;
  };

  Raster.prototype.leftExclusionEdge = function (y1, y2) {
    // y2 >= y1
    var intervals = this.intervals;
    ...
    for (var y = Math.max(y1, this.clip.y); y <= y2 && y < this.clip.maxY; y++) {
      ...
      if (x === undefined || startX !== undefined && startX < x) x = startX;
    }
  ...
  Rect.prototype.shiftRightEdgeTo = function (newX) {
    this.width = newX - this.x;
  };

  Rect.prototype.shiftBottomEdgeTo = function (newY) {
    this.height = newY - this.y;
  };
  ...
  function RoundedRect(rect, topLeft, topRight, bottomLeft, bottomRight) {
    // corner radii parameters are {width, height}
    this.rect = rect;
    this.radii = {
      topLeft: topLeft,
      topRight: topRight,
  ...
  RoundedRect.prototype.isEmpty = function () {
    return this.width <= 0 || this.height <= 0;
  };

  RoundedRect.prototype.topLeftCorner = function () {
    return new Rect(this.rect.x, this.rect.y, this.radii.topLeft.width, this.radii.topLeft.height);
  };

  RoundedRect.prototype.topRightCorner = function () {
    return new Rect(this.rect.maxX - this.radii.topRight.width, this.rect.y, this.radii.topRight.width, this.radii.topRight.height);
  };

  RoundedRect.prototype.bottomLeftCorner = function () {
    return new Rect(this.rect.x, this.rect.maxY - this.radii.bottomLeft.height, this.radii.bottomLeft.width, this.radii.bottomLeft.height);
  };

  RoundedRect.prototype.bottomRightCorner = function () {
    return new Rect(this.rect.maxX - this.radii.bottomRight.width, this.rect.maxY - this.radii.bottomRight.height, this.radii.bottomRight.width, this.radii.bottomRight.height);
  };

  RoundedRect.prototype.isRounded = function () {
    function isCornerRadiusNonZero(radius) {
      return radius.width > 0 && radius.height > 0;
    }

    return isCornerRadiusNonZero(this.radii.topLeft) || isCornerRadiusNonZero(this.radii.topRight) || isCornerRadiusNonZero(this.radii.bottomLeft) || isCornerRadiusNonZero(this.radii.bottomRight);
  };
  ...
  RoundedRect.prototype.scaleRadii = function (factor) {
    ...
    if (radii.topLeft.isEmpty()) radii.topLeft = Size.zeroSize;
    radii.topRight.scale(factor);
    if (radii.topRight.isEmpty()) radii.topRight = Size.zeroSize;
    radii.bottomLeft.scale(factor);
    if (radii.bottomLeft.isEmpty()) radii.bottomLeft = Size.zeroSize;
    radii.bottomRight.scale(factor);
    if (radii.bottomRight.isEmpty()) radii.bottomRight = Size.zeroSize;
  ...
  RoundedRect.prototype.isRenderable = function () {
    var radii = this.radii;
    var rect = this.rect;
    return radii.topLeft.width + radii.topRight.width <= rect.width && radii.bottomLeft.width + radii.bottomRight.width <= rect.width && radii.topLeft.height + radii.bottomLeft.height <= rect.height && radii.topRight.height + radii.bottomRight.height <= rect.height;
  ...
  RoundedRect.prototype.minXInterceptAt = function (y, noInterceptReturnValue) {
    if (!this.rect.containsY(y)) return noInterceptReturnValue;
    var topLeftCorner = this.topLeftCorner(),
        yi;
  ...
  RoundedRect.prototype.rightExclusionEdge = function (y1, y2) {
    // y2 >= y1
    if (this.rect.isEmpty() || !this.rect.overlapsYRange(y1, y2)) return undefined;
    if (!this.isRounded() || this.cornersInsetRect().overlapsYRange(y1, y2)) return this.rect.maxX;
    return Math.max(this.maxXInterceptAt(y1, this.rect.x), this.maxXInterceptAt(y2, this.rect.x));
  };

  RoundedRect.prototype.leftExclusionEdge = function (y1, y2) {
    // y2 >= y1
    if (this.rect.isEmpty() || !this.rect.overlapsYRange(y1, y2)) return undefined;
    if (!this.isRounded() || this.cornersInsetRect().overlapsYRange(y1, y2)) return this.rect.x;
  ...
  function computeOffsetHeight(dx, dy, areaLimit) {
    if (dy === 0) return 1;
    if (dx === 0 || dx * dy / 2 < areaLimit) return Math.round(dy);
    return Math.round(Math.sqrt(2 * areaLimit * (dy / dx)));
  }

  function leftCornerOffset(leftCorner, offset) {
    return leftCorner.maxX - offset;
  }

  function rightCornerOffset(rightCorner, offset) {
    return rightCorner.x + offset;
  }

  function leftBoxOffset(box) {
    return box.x;
  }

  function rightBoxOffset(box) {
    return box.maxX;
  }

  function adaptiveOffsetFnGenerator(getTopCorner, getBottomCorner, getBoxOffset, getCornerOffset) {
    return function (y1, y2, areaLimit) {
      // y2 >= y1
      if (!this.rect.overlapsYRange(y1, y2)) return [{
        x: undefined,
        height: y2 - y1
      }];
      var offsets = [];
      if (y1 < this.rect.y) offsets.push({
        x: undefined,
        height: this.rect.y - y1
      });
      var offsetHeight,
          minY,
          maxY,
      ...
      if (topCorner.overlapsYRange(y1, y2)) {
        ...
        for (y = minY; y < maxY; y += offsetHeight) {
          ...
          offsets.push({
            height: Math.min(offsetHeight, maxY - y),
            x: getCornerOffset(topCorner, xi)
          });
        }
      }
      ...
      if (boxArea.overlapsYRange(y1, y2)) offsets.push({
        x: getBoxOffset(boxArea),
        height: maxY - minY
      });
      ...
      if (bottomCorner.overlapsYRange(y1, y2)) {
        ...
        for (y = minY; y < maxY; y += offsetHeight) {
          ...
          offsets.push({
            height: Math.min(offsetHeight, maxY - y),
            x: getCornerOffset(bottomCorner, xi)
          });
        }
      }

      if (y2 > this.rect.maxY) offsets.push({
        x: undefined,
        height: y2 - this.rect.maxY
      });
      return offsets;
    };
  }

  RoundedRect.prototype.rightExclusionOffsets = adaptiveOffsetFnGenerator(RoundedRect.prototype.topRightCorner, RoundedRect.prototype.bottomRightCorner, rightBoxOffset, rightCornerOffset);
  RoundedRect.prototype.leftExclusionOffsets = adaptiveOffsetFnGenerator(RoundedRect.prototype.topLeftCorner, RoundedRect.prototype.bottomLeftCorner, leftBoxOffset, leftCornerOffset);
  ...
  function createRoundedRectForInset(inset, margin) {
    function toSize(r) {
      return new Size(r[0] + margin, r[1] + margin);
    }
  ...
  function createRoundedRectForBox(box, margin) {
    ...
    var topLeft = toSize(box.radii[0]),
        ...
        bottomLeft = toSize(box.radii[3]); // This box is at 0,0 relative to its sizing box (itself)
  ...
  function createShapeGeometry(shapeValue, whenReady) {
    ...
    if (shapeValue.shape) {
      switch (shapeValue.shape.type) {
        ...
        case "inset":
          ...
          if (!geometry.isRenderable()) geometry.adjustRadii();
          break;
    ...
    if (shapeValue.url) return createRaster(shapeValue.url, shapeValue.box, shapeValue.shapeImageThreshold, shapeMargin, shapeValue.clip, whenReady);
  ...
  ShapeInfo.prototype.onReady = function (callback) {
    if (this.ready) callback();else this.callback = callback;
  };

  ShapeInfo.prototype.leftExclusionEdge = function (line) {
    // { top, bottom, left, right }
    return this.geometry ? this.geometry.leftExclusionEdge(line.top, line.bottom) : line.left;
  };

  ShapeInfo.prototype.rightExclusionEdge = function (line) {
    // { top, bottom, left, right }
    return this.geometry ? this.geometry.rightExclusionEdge(line.top, line.bottom) : line.right;
  };

  function exclusionEdgeValue(x) {
  ...
  ShapeInfo.prototype.computeStepOffsets = function (step) {
    ...
    for (var i = 0; i < Math.ceil(this.metrics.marginBox.height / step); i++) {
      ...
      lineBounds.bottom -= this.metrics.margins[0] + this.shapeValue.box.y; // get the offset relative to the margin box
  ...
  ShapeInfo.prototype.computeAdaptiveOffsets = function (limit) {
    ...
    var offsets = this.metrics.cssFloat === 'left' ? this.geometry.rightExclusionOffsets(-dy, this.metrics.marginBox.height - dy, limit) : this.geometry.leftExclusionOffsets(-dy, this.metrics.marginBox.height - dy, limit);
    var result = [];
    var y = 0;

    for (var i = 0; i < offsets.length; i++) {
      var layoutOffset;
      if (offsets[i].x === undefined) layoutOffset = 0;else {
        layoutOffset = this.metrics.cssFloat == 'left' ? offsets[i].x + dx : this.metrics.marginBox.width - (offsets[i].x + dx);
        layoutOffset = Math.min(layoutOffset, this.metrics.marginBox.width);
      }
      result.push({
        offset: layoutOffset,
        top: y,
        bottom: y + offsets[i].height,
        cssFloat: this.metrics.cssFloat
      });
      y += offsets[i].height;
    }
  ...
  ShapeInfo.prototype.offsets = function (parameters) {
    if (this.geometry instanceof RoundedRect) return (parameters && parameters.mode) == "step" ? this.computeStepOffsets(parameters.step) : this.computeAdaptiveOffsets(parameters.limit);
  ...
  function Polyfill(scope) {
    ...
    var self = this;
    var autoRun = script.getAttribute('data-auto-run') !== 'false'; // IE < 9 uses attachEvent rather than addEventListener
  ...
  function fakeIt(element, offsets) {
    ...
    styles = {
      ...
      top: '0',
      width: '100%',
      // will fill the whole width, FF does 'auto' differently
    ...
    if (element.parentNode) element.parentNode.insertBefore(wrapper, element);
  ...
  Polyfill.prototype.polyfill = function (element, settings) {
    ...
    if (!(/left|right/.test(computedStyle.cssFloat) && element.getAttribute('data-shape-outside'))) return; // ideally this would default to lineHeight, but 'normal' is a valid computed value
    // and is up to the UA
    ...
    shapeInfo.onReady(function () {
      var offsets = shapeInfo.offsets({
        mode: mode,
        limit: limit,
        step: step
      });
      fakeIt(element, offsets);
      if (settings && settings.callback && typeof settings.callback === 'function') settings.callback.call(self.scope);
    });
  };
  ...
  Polyfill.prototype.removePolyfill = function (element) {
    ...
    for (oldParent = element.parentNode; oldParent && oldParent.hasAttribute && !oldParent.hasAttribute('data-shape-outside-container'); oldParent = oldParent.parentNode) {
      ;
    }

    if (!oldParent || !oldParent.hasAttribute) return;
  ...
  Polyfill.prototype.run = function (settings) {
    ...
    if (force === this.Force.LayoutStyles) settings.force = this.Force.Layout;else if (force) delete settings.force;
    ...
    if (this.hasNativeSupport && !force) return;
  ...
  Polyfill.prototype.Force = {
    Layout: 'force-layout',
    Styles: 'force-styles',
    LayoutStyles: 'force-layout-styles'
  };
  if (Object.freeze) Polyfill.prototype.Force = Object.freeze(Polyfill.prototype.Force);
  /**
  ...
  function adjustBounds(bounds, sign, offsets) {
    var top = offsets.reduce(function (prev, curr) {
      return prev + curr[0];
    }, 0);
    var right = offsets.reduce(function (prev, curr) {
      return prev + curr[1];
    }, 0);
    var bottom = offsets.reduce(function (prev, curr) {
      return prev + curr[2];
    }, 0);
    var left = offsets.reduce(function (prev, curr) {
      return prev + curr[3];
    }, 0);
    bounds.x -= sign * left;
    bounds.y -= sign * top;
  ...
  function adjustRadius(radius, sign, offset) {
    if (sign < 0) return Math.max(radius + sign * offset, 0);
    var ratio = Math.abs(radius / offset);
    if (ratio < 1) return Math.max(radius + offset * (1 + Math.pow(ratio - 1, 3)), 0);
  ...
  function adjustRadii(radii, sign, offsets) {
    ...
    var left = offsets.reduce(function (prev, curr) {
      return prev + curr[3];
    }, 0); // Still need to max these with 0
  ...
  ShapeValue.prototype.parseBox = function (text, metrics) {
    ...
    if (!box) box = 'margin-box';else box = box[0];
    var radii = JSON.parse(JSON.stringify(metrics.borderBox.radii));
    var result = {
      text: box,
      x: metrics.borderBox.x,
      y: metrics.borderBox.y,
  ...
  ShapeValue.prototype.printShape = function () {
    if (this.shape) {
      switch (this.shape.type) {
        case 'inset':
          return 'inset(' + this.shape.insets.join(' ') + ' round ' + pluck(this.shape.radii, 0).join(' ') + ' / ' + pluck(this.shape.radii, 1).join(' ') + ')';
        ...
        case 'polygon':
          return 'polygon(' + this.shape.fillRule + ', ' + this.shape.points.map(function (point) {
  ...
  ShapeValue.prototype.printBox = function () {
    if (this.box) {
      return this.box.text + ' { x: ' + this.box.x + ', y: ' + this.box.y + ', width: ' + this.box.width + ', height: ' + this.box.height + ', radii: ' + pluck(this.box.radii, 0).join(' ') + ' / ' + pluck(this.box.radii, 1).join(' ') + ' }';
    }
  ...
  function positionOffsetToPixels(offset, extent, metrics) {
    ...
    switch (offset[0]) {
      ...
      default:
        length = metrics.toPixels(offset[0], extent);
    }

    if (offset.length > 1) length = metrics.toPixels(offset[1], extent);
    return direction === 'TopLeft' ? length : extent - length;
  }

  function radiusToPixels(r, sides, extent, metrics) {
    if (r === 'closest-side') return Math.min.apply(null, sides);else if (r === 'farthest-side') return Math.max.apply(null, sides);else return metrics.toPixels(r, extent);
  ...
  ShapeValue.prototype.parseEllipsoid = function (args) {
    ...
    if (args && args[2]) {
      ...
      positions.forEach(function (position) {
        // if it is an offset
        if (/\d+/.test(position) && canMergeBack) resolvedPositions[resolvedPositions.length - 1] += ' ' + position;else resolvedPositions.push(position); // it's a non-center keyword and there are more than two inputs
  ...
  ShapeValue.prototype.parseCircle = function (args, box, metrics) {
    ...
    result.r = radiusToPixels(result.rx, [Math.abs(result.cx), Math.abs(box.width - result.cx), Math.abs(result.cy), Math.abs(box.height - result.cy)], Math.sqrt((box.width * box.width + box.height * box.height) / 2), metrics);
    delete result.rx;
    delete result.ry;
    return result;
  };
  ...
  ShapeValue.prototype.parsePolygon = function (args, box, metrics) {
    ...
    var points = args.map(function (point) {
      ...
      return {
        x: metrics.toPixels(coords[0], box.width),
        y: metrics.toPixels(coords[1], box.height)
      };
    });
    return {
      type: 'polygon',
  ...
  ShapeValue.prototype.computeClip = function (referenceBox, metrics) {
    ...
    return {
      x: -referenceBox.x - marginLeft,
      y: -referenceBox.y - marginTop,
      width: metrics.borderBox.width + marginWidth,
      height: metrics.borderBox.height + marginHeight
    };
  };
  ...
  function getStyleSheetElements() {
    ...
    if (typeof doc.querySelectorAll == 'function') {
      // shiny new browsers
      stylesheets = doc.querySelectorAll('link[rel="stylesheet"], style'); // make it an array
  ...
  StyleSheet.prototype.load = function (onSuccess, onError, scope) {
    var self = this; // Loading external stylesheet
